...
if __name__ == "__main__":
    try:
        print(f"KIS 환경: {'실전' if USE_PROD else '모의'} | BASE_URL: {BASE_URL} | TR_ID: {PRICE_TR_ID}")
        print(
            f"호출 제어: 최소간격={MIN_REQUEST_INTERVAL:.2f}s 최대간격={MAX_REQUEST_INTERVAL:.2f}s "
            f"재시도={RATE_LIMIT_RETRIES}회 백오프={RATE_LIMIT_BACKOFF:.2f}s"
        )
        # 1. 토큰 가져오기
        ...
        if auth_token:
            while True:
                ...
                for code, name in STOCKS.items():
                    results.append(get_current_price(auth_token, code, name))
                render_dashboard(results)
                time.sleep(POLL_LOOP_SLEEP)
