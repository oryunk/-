...
FAQ_DATA: list[dict[str, Any]] = [
    {
        ...
        "answer": (
            "처음이면 이렇게만 기억해 봐! ① 계좌 만들기 ② 왜 하는지 목표 잡기 "
            "③ 분산 투자 개념 익히기 ④ 소액으로 연습하기. 같이 천천히 가자~"
        ),
        "mood": "welcome",
    },
    {
        ...
        "answer": (
            "한 종목에 올인하지 않고 여러 곳에 나눠 담는 거야, 그게 분산투자! "
            "보통 10~20개 정도로 나눠 본다고 생각하면 이해하기 쉬워."
        ),
        "mood": "info",
    },
    {
        ...
        "answer": (
            "언제 사고팔지 맞추려다 지칠 수 있어. 장기로 보면서 "
            "매달 조금씩 넣는 것도 괜찮고, 좋은 회사면 단기 흔들림에 덜 놀라게 돼."
        ),
        "mood": "info",
    },
    {
        "keywords": ["손절", "손실", "하락"],
        "answer": (
            "손절은 미리 정해두는 게 마음 편해. 많은 사람들이 7~10% 정도 빠지면 "
            "다시 본다고 하더라. 감정 말고 내가 정한 룰대로 가봐."
        ),
        "mood": "caution",
    },
    {
        "keywords": ["배당", "배당주", "배당금"],
        "answer": (
            "배당주는 현금 흐름 받고 싶을 때 보면 좋아. 배당률·성장률 같이 보는데, "
            "무조건 배당만 높다고 좋은 건 아니야."
        ),
        "mood": "info",
    },
    {
        ...
        "answer": (
            "기술적 분석은 과거 가격 패턴을 보는 방법이야. 이동평균·RSI 등을 쓰는데 "
            "재무·뉴스 같은 기본 분석이랑 같이 보는 게 좋아."
        ),
        "mood": "info",
    },
    {
        ...
        "answer": (
            "손익계산서·재무상태표·현금흐름표로 기업 건전성을 볼 수 있어. "
            "PER, PBR, ROE, 부채비율 같은 게 대표 지표야."
        ),
        "mood": "info",
    },
    {
        "keywords": ["etf", "인덱스"],
        "answer": (
            "ETF는 여러 종목을 한 번에 담은 상품이라 처음 시작하는 사람한테도 편해. "
            "KOSPI200·S&P500 같은 지수 추종 상품부터 알아보고, 수수료도 비교해 봐."
        ),
        "mood": "success",
    },
    {
        ...
        "answer": (
            "국내 주식은 일반 투자자한테 양도소득세 면제인 경우가 많고, "
            "배당은 15.4% 원천징수가 일반적이야. 해외 주식은 별도 규정이 있으니 꼭 확인해 봐."
        ),
        "mood": "caution",
    },
    {
        ...
        "answer": (
            "여기 주린닷컴은 연습용 사이트야! 가이드로 배우고 시장에서 시세 보고, "
            "용어·AI분석·모의투자·뉴스도 다 쓸 수 있어. 막막하면 가이드부터 가 봐."
        ),
        "mood": "welcome",
    },
    {
...
FALLBACK_ANSWER = (
    "음, 그건 아직 내가 잘 모르겠어. 미안! "
    "주식 시작·분산투자·손절·배당·ETF·세금·사이트 쓰는 법 같은 걸로 다시 물어봐 줘~"
)
...
def _build_system_prompt(page_hint: str | None) -> str:
    ...
    return (
        ...
        "- 확인 안 된 사실은 단정하지 않음.\n"
        "- 주식·투자·경제·금융 외의 주제(연애, 정치, 의학 등)는 정중히 거절합니다.\n"
        f"{page_line}\n"
        "[구체성 — 가장 중요]\n"
        "- 절대로 '뉴스를 확인해보세요', '전문가에게 물어보세요', '공식 사이트를 참고하세요' 같은 "
        "  빈 말 금지. 이런 말은 아무 도움이 안 됩니다.\n"
        "- 개념 설명은 반드시 실제 예시를 들어 설명합니다. "
        "  (예: PER 설명 시 → 'PER 10이면 지금 이익의 10배 값을 주고 사는 거예요. "
        "  삼성전자 PER이 10이면 10년치 이익을 미리 주고 사는 셈')\n"
        "- 수치가 있으면 구체적인 수치로 설명합니다. "
        "  (예: '수수료가 낮다' → '키움증권 국내주식 수수료 0.015%'처럼)\n"
        "- 실시간 데이터(오늘 주가·최신 뉴스)는 모르지만, 알고 있는 배경 맥락을 구체적으로 설명합니다. "
        "  (예: 삼성전자 뉴스 질문 → 실시간 데이터는 없다고 솔직히 말하되, "
        "  HBM·반도체 업황·엔비디아 관계 등 배경 지식을 구체적으로 설명)\n"
        "- 질문이 복잡하거나 설명이 필요한 경우 3~6문장도 OK. 짧아도 알맹이가 있어야 합니다.\n"
        "[정확도]\n"
        "- 질문의 핵심 의도를 정확히 파악한 뒤 답합니다. 비슷해 보이는 질문도 다르게 답합니다.\n"
        "- 이전 대화에서 이미 한 말은 반복하지 않고 새로운 관점으로 답합니다.\n"
        "- 모르거나 불확실한 내용은 억지로 답하지 말고 '정확히는 모르겠지만' 등으로 솔직하게 말합니다.\n"
        "- 수치·날짜·법률·세금 정보는 '참고용'임을 명시하고 단정 짓지 않습니다.\n"
        "- 투자 손실 관련 질문엔 반드시 리스크를 언급합니다.\n"
